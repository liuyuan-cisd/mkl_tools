...
@LastEditTime: 2019-10-16 20:47:16
...
def averge_svm(train_x,train_y,test_x,test_y,c):
    ...
    m=K_train.shape[0]
    kernel_weights = get_easyMKl_weigths(train_x,train_y)
...
def get_easyMKl_weigths(train_x,train_y):
    klisttr=[matrix(k) for k in train_x]
    l=0.5 #lambda
    easy = EasyMKL(lam=l, tracenorm = True)
    easy.train(klisttr,matrix(train_y))
...
if __name__ == "__main__":
    ...
    y_true_list=list()
    y_proba_list=list()
    skflod=StratifiedKFold(n_splits=cv,random_state=0,shuffle=True)
    ...
    np.savetxt('easyMKL_SVM_result.csv',result,delimiter=',',fmt='%5f')
